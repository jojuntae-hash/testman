...
import { getLongTermBackupList, deleteLongTermBackups, getLongTermBackupData, saveLongTermBackup } from '@/lib/backupUtils'
import { useData } from '@/lib/DataContext'
import { X, Trash2, RotateCcw, AlertTriangle, Plus } from 'lucide-react'

interface LongTermBackupManagerModalProps {
  onClose: () => void
}

export default function LongTermBackupManagerModal({ onClose }: LongTermBackupManagerModalProps) {
  const [backups, setBackups] = useState<any[]>([])
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set())
  const [isLoading, setIsLoading] = useState(true)
  const { restoreLongTermFromBackup, longTermCustomers } = useData()

  const loadBackups = async () => {
    setIsLoading(true)
    try {
      const list = await getLongTermBackupList()
      setBackups(list)
    } catch (e) {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!confirm(`선택한 ${selectedIds.size}개의 장기 고객 백업을 삭제하시겠습니까?`)) return
    
    try {
      await deleteLongTermBackups(Array.from(selectedIds))
      setSelectedIds(new Set())
      await loadBackups()
  ...
  const handleRestore = async (id: string, name: string) => {
    if (!confirm(`[${name}] 백업으로 복원하시겠습니까?\n\n⚠️ 주의: 기존 데이터는 완전히 삭제되고 백업 시점의 장기 고객 데이터로 교체됩니다!`)) return
    
    try {
      const backupItem = await getLongTermBackupData(id)
      if (backupItem && backupItem.data) {
        await restoreLongTermFromBackup(backupItem.data)
  ...
  const handleManualBackup = async () => {
    if (!confirm('현재 장기 고객 데이터로 즉시 수동 백업을 생성하시겠습니까?')) return
    try {
      await saveLongTermBackup(longTermCustomers)
      localStorage.setItem('lastLongTermBackupTime', Date.now().toString())
      await loadBackups()
      alert('장기 고객 백업이 생성되었습니다.')
    } catch (e) {
