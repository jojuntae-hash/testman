...
import { getBackupList, deleteBackups, getBackupData, saveBackup } from '@/lib/backupUtils'
...
export default function BackupManagerModal({ onClose }: BackupManagerModalProps) {
  const [backups, setBackups] = useState<any[]>([])
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set())
  const [isLoading, setIsLoading] = useState(true)
  const { restoreFromBackup, customers } = useData()
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await deleteBackups(Array.from(selectedIds))
      setSelectedIds(new Set())
      await loadBackups()
  ...
  const handleRestore = async (id: string, name: string) => {
    if (!confirm(`[${name}] 백업으로 복원하시겠습니까?\n\n⚠️ 주의: 기존 데이터는 완전히 삭제되고 백업 시점의 데이터로 교체됩니다!`)) return
    
    try {
      const backupItem = await getBackupData(id)
      if (backupItem && backupItem.data) {
        await restoreFromBackup(backupItem.data)
  ...
  const handleManualBackup = async () => {
    if (!confirm('현재 데이터로 즉시 수동 백업을 생성하시겠습니까?')) return
    try {
      await saveBackup(customers)
      // 현재 시간을 lastBackupTime으로도 설정
