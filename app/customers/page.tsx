...
export default function CustomersPage() {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (cleanSearch) {
      list = list.filter(c => {
        ...
        const record = (c.기록 || '').replace(/\s+/g, '').toLowerCase()
  ...
  }, [longTermCustomers, searchTerm, selectedFolder, sortOption])
  ...
  const handleCreateNewFolder = () => {
    ...
    changeLongTermCustomerStatus(selectedIds, folderName)
  ...
  const uniqueExistingFolders = Array.from(new Set(longTermCustomers.map(c => c.status).filter(s => s && s !== '미분류')))
